...
import { mapMoviesList } from './map-movies-list'

type TGetMoviesSuccess = {
  type: 'success'
  data: TMovie[]
}

type TGetMoviesError = {
  type: 'error'
  error: any
}

type TGetMoviesResponse = TGetMoviesSuccess | TGetMoviesError
...
export const getMovies = async (
  ...
  try {
    ...
    const movieRes = await fetch(
      `${baseUrl}/3/search/movie?query=${searchQuery}&include_adult=false&language=en-US&page=1`,
      {
        method: 'GET',
    ...
    const data = mapMoviesList(movies)
...
// var mock = {
//   results: [movieMock, movieMock, movieMock, movieMock, movieMock],
// };
